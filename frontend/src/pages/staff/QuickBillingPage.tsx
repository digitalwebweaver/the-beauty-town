...
function QuickBillingPage() {
  ...
  // Mobile: the ticket lives in a tap-to-open bottom sheet instead of being
  // stacked at the end of the page — see the fixed bar + Sheet near the
  ...
  function addToCart(
    ...
    // Validated up front (against the current `cart`) purely so we know
    // whether to show a success or an error toast — the actual state
    // update below still goes through the functional form for safety.
    const key = `${type}:${refId}`;
    const existing = cart.find((l) => l.key === key);
    if (existing) {
      if (existing.maxStock !== undefined && existing.quantity + 1 > existing.maxStock) {
        toast.error(`Only ${existing.maxStock} left in stock`);
        return;
      }
    } else if (maxStock !== undefined && maxStock < 1) {
    ...
    // Instant confirmation on mobile, where the ticket itself is a tap
    // away in the bottom sheet rather than always in view — replaces
    // itself (same id) instead of stacking on a quick run of taps.
    toast.success(`${name} added — ${formatInr(unitPrice)}`, {
      id: 'qb-cart-toast',
      duration: 1200,
    });
  }
